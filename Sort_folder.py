...
def process_folder(folder_path):
    ...
    print(f"Sorting completed for folder: {folder_path}")
...
def main():
    ...
    # Замість виклику sort_folder безпосередньо, створюємо пул потоків і запускаємо обробку папки у паралельних потоках.
    with ThreadPoolExecutor(max_workers=5) as executor:  # Кількість потоків = 5
